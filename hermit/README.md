...
Run `lock` (or `Super + Ctrl + L`) for the lock screen to appear. You can
change `$wallpaper`, `$clock_font` and `$ui_font` at the top of
`.config/hypr/hyprlock.conf` to customize it.
