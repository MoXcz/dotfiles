...
// Displays panel backed by hyprmoncfg (https://github.com/crmne/hyprmoncfg).
//
// Ported from omarchy-hyprmoncfg (MIT, Carmine Paolino,
// https://github.com/crmne/omarchy-hyprmoncfg); see LICENSE next to this
// file. The port hosts the panel in our BarPopup instead of omarchy's
// KeyboardPanel, styles through the Style/Color/Border shims in qs.Commons,
// and drops the omarchy plugin installer/updater. Everything else — layout
// canvas, display/color inspector, profile browser, workspace planner, safe
// preview with countdown, keyboard navigation — is the original design.
//
// Talks to hyprmoncfgd over $XDG_RUNTIME_DIR/hyprmoncfgd.sock (protocol 1)
// and subscribes to status events; PreviewGuard owns the Keep/Revert dialog
// so it survives the monitor change the preview causes.
BarPopup {
  id: root
  ...
  readonly property bool backendConnected: backendSocket.connected
  ...
  // ------------------------------------------------------------ geometry
  // Compact is a fixed column; expanded grows to the editor size but never
  // past the screen the bar sits on (the card adds Theme.spaceLg padding).
  readonly property var hostScreen: anchorItem && anchorItem.QsWindow.window ? anchorItem.QsWindow.window.screen : null
  readonly property int screenW: hostScreen ? hostScreen.width : 1920
  readonly property int screenH: hostScreen ? hostScreen.height : 1080
  readonly property int compactWidth: 430
  readonly property int expandedWidth: Math.min(1120, screenW - Theme.gapsOut * 2)
  readonly property int expandedHeight: Math.min(780, screenH - Config.bar.height - Theme.gapsOut * 2 - Theme.spaceSm)
  panelWidth: expanded ? expandedWidth : compactWidth
  ...
  readonly property string pendingProfileName: root.daemonPreview ? String(root.daemonPreview.profile_name || "") : ""
  ...
  readonly property string exactDisplayProfileName: root.exactDisplayProfile ? String(root.exactDisplayProfile.name || "") : ""
  ...
  readonly property string displayedProfile: pendingProfileName !== ""
    ? pendingProfileName
    : (profileOverride !== "" ? profileOverride : (activeProfile !== "" ? activeProfile : recommendedProfile))
  ...
  readonly property bool daemonOutdated: root.backendConnected && root.documentReady
    && Model.daemonNeedsRestart(root.installedVersion, root.document ? root.document.version : "")
  // Every actionable compact row in one list so cursor positions match what
  // is on screen.
  readonly property var actionRows: {
    var rows = []
    if (root.compatible && root.serviceStateKnown && !root.serviceEnabled && !root.serviceActive && !root.serviceActionPending)
      rows.push({ id: "enable-service", icon: "󰐊", title: "Enable daemon",
        subtitle: "systemctl --user enable --now hyprmoncfgd" })
    if (root.serviceBroken)
      rows.push({ id: "restart-service", icon: "󰑓", title: "Restart hyprmoncfg",
        subtitle: "Try the background service again" })
    else if (root.daemonOutdated)
      rows.push({ id: "restart-service", icon: "󰑓", title: "Restart daemon",
  ...
  readonly property bool serviceBroken: serviceStateKnown && serviceEnabled && !backendConnected
    && !connectionGrace && !serviceActionPending
  ...
  readonly property var previewCoordinator: PreviewGuard
  readonly property bool barIconDimmed: root.installationStateKnown && root.compatible && root.serviceStateKnown
    && !root.managedChecked && !root.serviceActionPending
  ...
  // NumberField is backed by a QML int; that is the only ceiling.
  ...
  readonly property var brightnessTarget: Model.brightnessTarget(root.draftProfile, root.selectedOutputKey, root.editorDocument.displays)
  ...
  // Internal panels go through brightnessctl; externals need ddcutil.
  readonly property bool brightnessShown: root.brightnessConnector !== ""
    && (Model.isInternalConnector(root.brightnessConnector) || root.ddcutilAvailable)
  ...
  readonly property int selectedSavedDetailRowCount: 5 + root.selectedSavedMatchReasons.length
    + root.selectedSavedHiddenRows.length + Math.max(1, root.selectedSavedWorkspaceRows.length)
  ...
  function close() {
    // Without the guard, an unanswered preview would silently expire.
    if (root.previewTransaction !== "" && !root.previewCoordinatorReady("keep")) root.revertPreview()
  ...
  function refreshBrightness() {
    ...
    if (!root.opened || connector === "" || !root.brightnessShown) {
    ...
    brightnessReadProcess.command = Model.brightnessReadArgs(connector)
  ...
  function startBrightnessSet(connector, percent) {
    ...
    brightnessSetProcess.command = Model.brightnessSetArgs(connector, percent)
  ...
  function launchTui() {
    Util.execArgv([Config.terminal, "-e", "hyprmoncfg"])
    root.close()
  }
  ...
  function send(method, params, context) {
    ...
    var request = { type: "request", protocol_version: 1, id: id, method: method }
  ...
  function moveManualWorkspace(row, delta) {
    ...
    root.editWorkspaces({ rules: Model.cycleManualWorkspaceRule(settings.rules, root.draftProfile, row, delta) })
    root.manualWorkspaceRulesInitialized = true
  }
  ...
  function nudgeSelectedOutput(dx, dy) {
    ...
    root.editOutput({ x: Number(output.x || 0) + Number(dx || 0), y: Number(output.y || 0) + Number(dy || 0) })
  }
  ...
  function cycleLayoutKeyboardPane(direction) {
    ...
    if (root.keyboardLayoutPane !== "canvas") {
      ...
      var fields = root.keyboardLayoutPane === "display" ? root.displayKeyboardFields : root.colorKeyboardFields
      if (fields.indexOf(root.keyboardInspectorField) < 0) root.keyboardInspectorField = fields[0]
    }
  }

  function moveInspectorCursor(delta) {
    var fields = root.inspectorPage === "display" ? root.displayKeyboardFields : root.colorKeyboardFields
  ...
  function inspectorHasCursor(field) {
    return root.expanded && root.activePage === "layout"
      && root.keyboardLayoutPane !== "canvas" && root.keyboardInspectorField === field
  }

  function bounded(value, minimum, maximum) { return Math.max(minimum, Math.min(maximum, value)) }
  ...
  function adjustInspectorField(delta) {
    ...
    else if (field === 3) edit.bitdepth = Number(Model.cycleOptionValue(root.bitdepthOptions, String(output.bitdepth || 8), delta))
    else if (field === 4) edit.cm = Model.cycleOptionValue(root.colorManagementOptions, String(output.cm || "srgb"), delta)
    else if (field === 5) edit.vrr = Number(Model.cycleOptionValue(root.vrrOptions, String(output.vrr || 0), delta))
    else if (field === 6) edit.transform = Number(Model.cycleOptionValue(root.transformOptions, String(output.transform || 0), delta))
    ...
    else if (field === 14) edit.sdr_eotf = Model.cycleOptionValue(sdrCurveDropdown.options, String(output.sdr_eotf || "default"), delta)
    ...
    else if (field === 18) edit.supports_wide_color = Number(Model.cycleOptionValue(root.triStateOptions, String(output.supports_wide_color || 0), delta))
    else if (field === 19) edit.supports_hdr = Number(Model.cycleOptionValue(root.triStateOptions, String(output.supports_hdr || 0), delta))
    else return
  ...
  function selectSavedProfile(delta) {
    var profiles = root.editorDocument && root.editorDocument.profiles instanceof Array ? root.editorDocument.profiles : []
  ...
  function workspaceKeyboardCount() {
    var settings = ((root.draftProfile || {}).workspaces || {})
    if (root.workspaceStrategy === "manual") return root.workspaceListKeyboardStart + root.manualWorkspaceRows.length
    var order = settings.monitor_order || []
    return root.workspaceListKeyboardStart + order.length
  }

  function normalizeWorkspaceCursor() {
    root.workspaceKeyboardIndex = Model.wrapIndex(root.workspaceKeyboardIndex, root.workspaceKeyboardCount())
    if (manualAssignmentList.visible && root.workspaceKeyboardIndex >= root.workspaceListKeyboardStart)
      manualAssignmentList.positionViewAtIndex(root.workspaceKeyboardIndex - root.workspaceListKeyboardStart, ListView.Contain)
  }

  function moveWorkspaceCursor(delta) {
    root.workspaceKeyboardIndex = Model.wrapIndex(root.workspaceKeyboardIndex + delta, root.workspaceKeyboardCount())
    if (manualAssignmentList.visible && root.workspaceKeyboardIndex >= root.workspaceListKeyboardStart)
      manualAssignmentList.positionViewAtIndex(root.workspaceKeyboardIndex - root.workspaceListKeyboardStart, ListView.Contain)
  }
  ...
  function adjustWorkspaceKeyboard(delta) {
    ...
    if (index === 0) root.editWorkspaces({ enabled: !settings.enabled })
      ...
      Model.cycleOptionValue(["manual", "sequential", "interleave"], String(settings.strategy || "manual"), delta))
      ...
      root.editWorkspaces({ group_size: Math.floor(root.bounded(Number(settings.group_size || 3) + delta, 1, root.workspaceValueMaximum)) })
    } else {
      if (String(settings.strategy || "") === "manual") {
  ...
  function yieldPreviewToPanel() {
    if (root.previewCoordinator && typeof root.previewCoordinator.yieldToPanel === "function")
  ...
  function previewDraft() {
    ...
    if (root.previewCoordinatorReady("startDraftPreview")) {
      if (!root.previewCoordinator.startDraftPreview(Model.namedProfile(root.draftProfile, name), 10)) {
        root.previewPending = false
        root.lastError = String(root.previewCoordinator.errorMessage || "Could not open the display confirmation.")
      }
      return
    }
  ...
  function applyDraft() {
    ...
    root.yieldPreviewToPanel()
    root.send("preview", { profile: profile, timeout_seconds: 10, save_on_commit: false }, { kind: "draft-apply" })
  }
  ...
  function handleExpandedActivate(returnPressed) {
    if (root.keyboardHelpOpen) { root.keyboardHelpOpen = false; return }
    if (root.previewTransaction !== "") { root.keepPreview(); return }
    if (root.activePage === "layout") {
      if (root.keyboardLayoutPane === "canvas") {
  ...
  function handleExpandedText(text) {
    ...
    if (key === "?") { root.keyboardHelpOpen = true; return }
    if (key === "q") { root.close(); return }
    if (key === "R") { if (root.daemonOutdated) root.restartService(); return }
    if (key === "r") { root.requestEditorState(); return }
    if (key === "s") { root.keyboardSaveDraft(); return }
    if (key === "a") {
  ...
  function previewProfile(name) {
    ...
    if (selected === "" || !root.managedChecked) return
    ...
    root.yieldPreviewToPanel()
    root.send("preview", { profile_name: selected, timeout_seconds: 10 }, { kind: "profile", name: selected })
  }
  ...
  function keepPreview() {
    ...
    root.previewPending = true
    if (root.previewCoordinator && String(root.previewCoordinator.transactionId || "") === root.previewTransaction) {
      if (!root.previewCoordinator.keep()) root.previewPending = false
      return
    }
    root.send("commit", { transaction_id: root.previewTransaction, save: root.previewKind === "draft" }, { kind: root.previewKind })
  }
  ...
  function syncDaemonPreview(pending) {
    ...
    if (id !== "") {
      ...
      previewTimer.start()
      if ((!root.previewCoordinator || !root.previewCoordinator.connected || root.previewCoordinator.yieldingToPanel)
  ...
  function handleMessage(line) {
    ...
    } else if (method === "editor_state") {
      root.updateEditor(envelope.result)
  ...
  function activateCursor() {
    ...
    if (root.cursorIndex === 0) { root.setManaged(!root.managedChecked); return }
    var row = root.actionRows[root.cursorIndex - 1]
    if (row) { root.activateRow(String(row.id)); return }
    root.launchTui()
  }

  function activateRow(id) {
    if (id === "restart-service") root.restartService()
    else if (id === "enable-service") root.setManaged(true)
  }

  Component.onCompleted: {
    root.checkInstallation()
    ddcutilProbe.running = true
  }

  onActivePageChanged: {
    if (root.activePage === "workspaces") Qt.callLater(function() { root.ensureManualWorkspaceRules() })
  }
  ...
  Socket {
    ...
    onConnectedChanged: {
      if (connected) {
        ...
        if (root.compatible && (root.serviceEnabled || root.serviceActive)) serviceRefreshTimer.restart()
      }
    }
  ...
  Process {
    id: whichProcess
    command: ["sh", "-c", "command -v hyprmoncfg >/dev/null 2>&1 || exit 1; hyprmoncfg version"]
    ...
    onExited: function(exitCode) {
      ...
      root.installedVersion = root.installed ? String(versionOutput.text || "") : ""
      root.compatible = root.installed && Model.versionAtLeast(versionOutput.text, "1.16.2")
      if (root.compatible) root.checkServiceState()
      else {
  ...
  Process {
    ...
    onExited: function(exitCode) { root.ddcutilAvailable = exitCode === 0 }
  }
  ...
  Process {
    ...
    onExited: function(exitCode) {
      ...
      if (root.serviceActionPending && !serviceProcess.running) {
        // Only the managed direction is visible to systemctl; unmanage is
        // confirmed by the daemon's status document in updateDocument.
        var confirmed = root.serviceTargetManaged && root.serviceEnabled && root.serviceActive
        if (confirmed) {
          root.serviceActionPending = false
          root.serviceAction = ""
          serviceConfirmationTimer.stop()
  ...
  Process {
    ...
    onExited: function(exitCode) {
      ...
      if (exitCode !== 0) {
        root.serviceActionPending = false
        root.serviceAction = ""
        var fallback = action === "disable" ? "Could not hand display control back to Hyprland." : "Could not start hyprmoncfg."
  ...
  Timer { id: brightnessSelectionTimer; interval: 80; onTriggered: root.refreshBrightness() }
  Timer { id: brightnessSetDebounce; interval: 180; onTriggered: root.setBrightness(root.brightnessPercent) }

  Timer {
    interval: 5000
    repeat: true
    running: root.opened && root.brightnessShown
  ...
  Process {
    ...
    onExited: function(exitCode) {
      ...
      if (exitCode !== 0 && completedConnector === root.brightnessConnector) root.brightnessAvailable = false
      ...
      if (root.brightnessReadQueued) {
        root.brightnessReadQueued = false
        // Reading right after a write can return the stale value; a new
        // selection needs a read now, the same display can wait for the
        // periodic refresh.
        if (completedConnector !== root.brightnessConnector || exitCode !== 0) brightnessSelectionTimer.restart()
      }
    }
  }

  Timer { id: serviceRefreshTimer; interval: 250; onTriggered: root.checkServiceState() }
  ...
  Timer { id: connectionGraceTimer; interval: 2000; onTriggered: root.connectionGrace = false }
  ...
  Timer {
    ...
    interval: 1000
    repeat: true
    running: root.compatible && (root.serviceActive || (root.serviceActionPending && root.serviceTargetManaged)) && !root.backendConnected
    onTriggered: {
      root.checkServiceState()
      root.connectBackend()
    }
  ...
  Timer { id: previewTimer; interval: 250; repeat: true; onTriggered: root.updatePreviewClock() }

  // Applying a layout can rebuild the per-screen bar and destroy the popup
  // that started the preview. The daemon keeps the transaction alive; the
  // replacement widget reopens so the Keep/Revert choice is reachable when
  // the guard service is not there to show it.
  Timer {
    ...
    onTriggered: {
      attempts++
      if (root.previewTransaction === "" || root.opened) { stop(); return }
      if (root.anchorItem) root.open()
      if (attempts >= 20) stop()
    }
  }
  ...
  Item {
    id: frame
    width: parent.width
    height: root.expanded ? root.expandedHeight - Theme.spaceLg * 2 : compactColumn.implicitHeight

    // Modified arrows for fine nudges. The key catcher leaves modified arrows
    // alone so these can claim them.
    Item {
      id: shortcuts
      width: 0
      height: 0
      readonly property bool canvasKeys: root.opened && root.expanded && root.activePage === "layout"
        && root.keyboardLayoutPane === "canvas" && !root.keyboardHelpOpen && !root.execEditing && !keyCatcher.blocked

      Shortcut { sequence: "Shift+Left"; enabled: shortcuts.canvasKeys; onActivated: root.nudgeSelectedOutput(-10, 0) }
      Shortcut { sequence: "Shift+Right"; enabled: shortcuts.canvasKeys; onActivated: root.nudgeSelectedOutput(10, 0) }
      Shortcut { sequence: "Shift+Up"; enabled: shortcuts.canvasKeys; onActivated: root.nudgeSelectedOutput(0, -10) }
      Shortcut { sequence: "Shift+Down"; enabled: shortcuts.canvasKeys; onActivated: root.nudgeSelectedOutput(0, 10) }
      Shortcut { sequence: "Ctrl+Left"; enabled: shortcuts.canvasKeys; onActivated: root.nudgeSelectedOutput(-1, 0) }
      Shortcut { sequence: "Ctrl+Right"; enabled: shortcuts.canvasKeys; onActivated: root.nudgeSelectedOutput(1, 0) }
      Shortcut { sequence: "Ctrl+Up"; enabled: shortcuts.canvasKeys; onActivated: root.nudgeSelectedOutput(0, -1) }
      Shortcut { sequence: "Ctrl+Down"; enabled: shortcuts.canvasKeys; onActivated: root.nudgeSelectedOutput(0, 1) }
      Shortcut { sequence: "Alt+Left"; enabled: shortcuts.canvasKeys; onActivated: root.snapSelectedOutput("left") }
      Shortcut { sequence: "Alt+Right"; enabled: shortcuts.canvasKeys; onActivated: root.snapSelectedOutput("right") }
      Shortcut { sequence: "Alt+Up"; enabled: shortcuts.canvasKeys; onActivated: root.snapSelectedOutput("up") }
      Shortcut { sequence: "Alt+Down"; enabled: shortcuts.canvasKeys; onActivated: root.snapSelectedOutput("down") }
      Shortcut {
        sequence: "L"
    ...
    PanelKeyCatcher {
      ...
      onTabRequested: function(direction) { if (root.expanded) root.handleExpandedTab(direction) }
      ...
      Column {
        ...
        Item {
          ...
          Column {
            ...
            Text {
              textFormat: Text.PlainText
              width: parent.width
              text: root.installedRelease !== "" ? "hyprmoncfg " + root.installedRelease : "hyprmoncfg"
              color: root.dim
              font.family: root.fontFamily
              font.pixelSize: Style.font.caption
              font.bold: true
        ...
        // Not installed / too old. Installation is the package manager's job.
        Column {
          ...
          Text {
            ...
            width: parent.width
            text: root.installed
              ? "hyprmoncfg " + (root.installedRelease || "?") + " is too old; 1.16.2 or newer is required."
              : "hyprmoncfg is not installed."
            color: root.foreground
            font.family: root.fontFamily
          ...
          Text {
            textFormat: Text.PlainText
            width: parent.width
            text: root.installed ? "yay -Syu hyprmoncfg-bin" : "yay -S hyprmoncfg-bin"
            color: root.dim
            font.family: root.fontFamily
            font.pixelSize: Style.font.bodySmall
          }
          ...
          Text {
            textFormat: Text.PlainText
            width: parent.width
            text: "Then enable the daemon from this panel."
            color: root.dim
            font.family: root.fontFamily
            font.pixelSize: Style.font.caption
            wrapMode: Text.WordWrap
        ...
        Column {
          ...
          Column {
            ...
            PanelSectionHeader { text: "MONITOR MANAGEMENT" }
            ...
            Toggle {
              ...
              description: {
                ...
                if (root.managedChecked && root.profileAutomatic) return "Switch layouts on monitor, lid, and resume events"
          ...
          BorderSurface {
            ...
            Row {
              ...
              Button {
                ...
                bordered: true
                enabled: root.previewTransaction !== "" ? !root.previewPending : (!root.editorLoading && !root.editPending)
                fontSize: Style.font.caption
              ...
              Button {
                ...
                text: root.previewTransaction !== "" ? "Keep" : (root.sourceProfile !== "" ? "Preview" : "Finish in editor")
                selected: true
                bordered: true
                enabled: !root.editPending && !root.previewPending && root.managedChecked
                fontSize: Style.font.caption
                horizontalPadding: Style.space(8)
                verticalPadding: Style.space(4)
                onClicked: {
          ...
          Column {
            ...
            PanelSectionHeader { text: "PROFILE" }
            ...
            Button {
              ...
              selected: true
              bordered: true
              enabled: root.managedChecked && !root.profileModePending && root.previewTransaction === "" && !root.previewPending
              onClicked: root.setProfileAutomatic(true)
            }

            Button {
              width: parent.width
              visible: root.profileAutomatic && root.documentReady && root.connectedDisplayCount > 0
                && !root.exactDisplayProfile && root.previewTransaction === "" && !root.previewPending
              text: "Create profile"
              selected: true
              bordered: true
      ...
      Item {
          ...
          Row {
            ...
            Text {
              ...
              text: "Current setup  ·  " + root.profileStatusTitle
                + (!root.managedChecked ? " · read-only" : (root.profileAutomatic ? " · automatic" : " · pinned"))
              color: root.dim
              font.family: root.fontFamily
              font.pixelSize: Style.font.body
              elide: Text.ElideRight
            ...
            Column {
              ...
              EditorPane {
                ...
                Column {
                  ...
                  InfoRow { label: "Connector"; value: root.selectedOutput ? String(root.selectedOutput.name || "—") : "—" }
                  InfoRow { label: "Type"; value: Model.displayType(root.selectedOutputMetadata, root.selectedOutput) }
                  InfoRow { label: "Model"; value: root.selectedOutput ? Model.displayModelLabel(root.selectedOutput, false) : "—" }
                  InfoRow {
                    label: "Serial"
                  ...
                  InfoRow { label: "Workspace"; value: String(root.selectedOutputMetadata.workspace || "(none)") }
                  InfoRow { label: "DPMS"; value: Model.onOff(root.selectedOutputMetadata.dpms === true) }
                  InfoRow {
                    visible: Number(root.selectedOutputMetadata.physical_width || 0) > 0
                    label: "Panel mm"
                    value: Number(root.selectedOutputMetadata.physical_width || 0) + " × " + Number(root.selectedOutputMetadata.physical_height || 0) + " mm"
                  }
                }
              }
              ...
              EditorPane {
                ...
                Column {
                  ...
                  PanelSeparator { visible: root.brightnessShown; width: parent.width }
                ...
                Column {
                  ...
                  Grid {
                    ...
                    PanelDropdown {
                      ...
                      value: root.selectedOutput && String(root.selectedOutput.cm || "") !== "" ? String(root.selectedOutput.cm) : "srgb"
                      enabled: !!root.selectedOutput && !root.editPending
                  ...
                  Column {
                    ...
                    PanelSectionHeader { text: "ICC PROFILE" }
            ...
            EditorPane {
              ...
              Column {
                ...
                Toggle {
                  ...
                  description: root.profileModePending ? "Updating profile selection mode…"
                    : "Matches your connected displays to your saved profiles"
                  checked: root.profileAutomatic
                  enabled: root.managedChecked && !root.profileModePending && root.previewTransaction === ""
                    && !root.previewPending && (!root.profileAutomatic || root.activeProfile !== "")
                ...
                Repeater {
                  ...
                  BorderSurface {
                    ...
                    borderSpec: selected ? Border.controlSpec("selected") : Border.none()
                    radius: Style.cornerRadius
                    ...
                    Row {
                      ...
                      Text {
                        ...
                        color: profileRow.modelData.active || profileRow.selected ? root.foreground : root.dim
                        font.family: root.fontFamily
                        font.pixelSize: Style.font.bodySmall
                    ...
                    MouseArea {
                      anchors.fill: parent
                      hoverEnabled: true
                      enabled: String(profileRow.modelData.name || "") !== ""
                      cursorShape: Qt.PointingHandCursor
                      onClicked: root.selectedSavedProfileName = String(profileRow.modelData.name || "")
                    }
                  }
                }
              }
              ...
              EditorPane {
                ...
                Column {
                  ...
                  InfoRow {
                    ...
                    valueAccent: !!root.selectedSavedSummary && (root.selectedSavedSummary.active || root.selectedSavedSummary.recommended)
                  }
                  ...
                  InfoRow {
                    ...
                    value: root.selectedSavedSummary
                      ? Number(root.selectedSavedSummary.output_count || 0) + " saved · " + Number(root.selectedSavedSummary.connected_outputs || 0) + " connected"
                      : "—"
                  }
            ...
            EditorPane {
              ...
              Column {
                ...
                Toggle {
                  ...
                  hasCursor: root.expanded && root.activePage === "workspaces" && root.workspaceKeyboardIndex === 0
                ...
                PanelDropdown {
                  ...
                  enabled: root.editorReady && !root.editPending
                  hasCursor: root.expanded && root.activePage === "workspaces" && root.workspaceKeyboardIndex === 1
                ...
                Row {
                  ...
                  NumberField {
                    ...
                    width: root.workspaceGroupSizeApplicable ? (parent.width - parent.spacing) / 2 : parent.width
                    fieldWidth: width
                    label: "WORKSPACES"
                    from: 1
                    ...
                    hasCursor: root.expanded && root.activePage === "workspaces" && root.workspaceKeyboardIndex === 2
                    onModified: function(value) {
                  ...
                  NumberField {
                    ...
                    enabled: !root.editPending && String(((root.draftProfile || {}).workspaces || {}).strategy || "") === "sequential"
                    hasCursor: root.expanded && root.activePage === "workspaces" && root.workspaceKeyboardIndex === 3
                    onModified: function(value) {
                ...
                Repeater {
                  ...
                  BorderSurface {
                    ...
                    readonly property bool hasKeyboardCursor: root.expanded && root.activePage === "workspaces"
                      && root.workspaceKeyboardIndex === root.workspaceListKeyboardStart + index
                    color: hasKeyboardCursor ? Theme.selected : Theme.surfaceAlt
                    ...
                    Row {
                      ...
                      Button {
                        ...
                        text: "→"
                        bordered: true
                        enabled: orderRow.index < (((root.draftProfile || {}).workspaces || {}).monitor_order || []).length - 1 && !root.editPending
                        onClicked: root.moveWorkspaceMonitor(String(orderRow.modelData), 1)
                      }
                    }
                  }
                ...
                ListView {
                  ...
                  ScrollBar.vertical: ScrollBar { id: manualScrollBar; policy: ScrollBar.AsNeeded }

                  delegate: BorderSurface {
                    id: manualRow
                    required property var modelData
                    required property int index
                    width: manualAssignmentList.width - (manualScrollBar.visible ? manualScrollBar.width + Style.space(4) : 0)
                    height: Style.space(42)
                    readonly property bool hasKeyboardCursor: root.expanded && root.activePage === "workspaces"
                      && root.workspaceKeyboardIndex === root.workspaceListKeyboardStart + index
                    color: hasKeyboardCursor ? Theme.selected : Theme.surfaceAlt
                    ...
                    Row {
                      ...
                      Text {
                        textFormat: Text.PlainText
                        anchors.verticalCenter: parent.verticalCenter
                        width: parent.width - manualWorkspaceLabel.width - manualLeft.width - manualRight.width - parent.spacing * 3
                        text: String(manualRow.modelData.display_name || "Display")
                        color: root.foreground
                        font.family: root.fontFamily
                        font.pixelSize: Style.font.body
                        font.bold: manualRow.hasKeyboardCursor
                        elide: Text.ElideRight
                      }
        ...
        BorderSurface {
          ...
          borderSpec: Border.controlSpec(root.draftDirty || root.creatingProfile ? "selected" : "normal")
          radius: Style.cornerRadius
          opacity: root.managedChecked ? 1.0 : root.unmanagedOpacity
            ...
            Column {
                ...
                text: root.lastError !== ""
                  ...
                  : (root.creatingProfile ? "Creating a profile for this setup"
                    ...
                    : (root.activePage === "profiles"
                      ? (root.selectedSavedSummary && root.selectedSavedSummary.active ? "This profile is active" : "Browsing " + root.selectedSavedProfileName)
              ...
              Text {
                ...
                text: root.editPending ? "Checking layout…"
                  ...
                  : (root.activePage === "profiles"
                    ? (root.profileAutomatic ? "Turn off automatic selection to activate a profile." : "Activation uses a safe 10-second preview.")
      ...
      // Exec editor for the selected saved profile.
      Item {
        anchors.fill: parent
        z: 110
        ...
        BorderSurface {
          ...
          color: Theme.surface
          borderSpec: Border.controlSpec("focus")
          radius: Style.cornerRadius
  ...
  component DecimalField: Column {
    ...
    onValueChanged: if (!decimalInput.activeFocus) decimalInput.text = decimalField.formatted()

    PanelSectionHeader { text: decimalField.label }
