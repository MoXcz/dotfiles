// Pure helpers for the displays panel. Ported from omarchy-hyprmoncfg
// (MIT, Carmine Paolino) with the omarchy plugin-updater and installer
// helpers removed; this profile installs hyprmoncfg through the package manager.
...
// ---------------------------------------------------------------- brightness
// omarchy shipped `omarchy-brightness-display`; this shell reads the internal
// panel through brightnessctl and external displays through ddcutil when it
// is installed. `ddcutil detect --brief` lists one block per display with a
// "DRM connector: cardN-<connector>" line, which is how a Hyprland output
// name maps to a ddcutil display number.
function isInternalConnector(name) {
  return /^(eDP|LVDS|DSI)-/i.test(String(name || ""))
}

// Shell snippet printing an integer percent for the connector in $1, exit 1
// when unavailable. Kept in one place so read/write share the ddcutil lookup.
function brightnessReadArgs(connector) {
  return ["sh", "-c",
    'c="$1"; ' +
    'case "$c" in eDP-*|LVDS-*|DSI-*) ' +
    '  out=$(brightnessctl -m 2>/dev/null) || exit 1; ' +
    '  printf "%s\n" "$out" | head -n1 | cut -d, -f4 | tr -d "%"; exit 0;; esac; ' +
    'command -v ddcutil >/dev/null 2>&1 || exit 1; ' +
    'n=$(ddcutil detect --brief 2>/dev/null | awk -v c="$c" \'/^Display [0-9]+/{d=$2} /DRM connector:/{ if ($3 ~ ("-" c "$")) { print d; exit } }\'); ' +
    '[ -n "$n" ] || exit 1; ' +
    'ddcutil -d "$n" getvcp 10 --brief 2>/dev/null | awk \'{ print $4; exit }\'',
    "sh", String(connector || "")]
}

function brightnessSetArgs(connector, percent) {
  return ["sh", "-c",
    'c="$1"; p="$2"; ' +
    'case "$c" in eDP-*|LVDS-*|DSI-*) exec brightnessctl -q set "$p%";; esac; ' +
    'command -v ddcutil >/dev/null 2>&1 || exit 1; ' +
    'n=$(ddcutil detect --brief 2>/dev/null | awk -v c="$c" \'/^Display [0-9]+/{d=$2} /DRM connector:/{ if ($3 ~ ("-" c "$")) { print d; exit } }\'); ' +
    '[ -n "$n" ] || exit 1; ' +
    'exec ddcutil -d "$n" setvcp 10 "$p"',
    "sh", String(connector || ""), String(clampBrightness(percent))]
}
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    brightnessReadArgs: brightnessReadArgs,
